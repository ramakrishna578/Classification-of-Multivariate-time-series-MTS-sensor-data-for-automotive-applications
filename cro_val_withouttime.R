...
# adding row names
row.names(n) <- n$y
# delete the column contatining names in this case it is 6
n <- n[,-6]
...
for(i in 1:l){
  ...
  # designing NN
  NN <- nnet(training[,-5],idc,size = 10,maxit = 200,softmax = T)
  # predicting class of tst data
  nn1 <- predict(NN,test[,-5],type = "class")
  # classification accuracy
  # classification accuracy
 print(paste(table(predict(NN,test[,-5],type = "class"),test$Verhalten)))
